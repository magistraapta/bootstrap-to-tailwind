  ...
  $(document).on("submit", "#ajaxLogin", function (event) {
    ...
    if (event.target.getAttribute("data-code") == "redirectNext") {
      if (url === "front.courses.play") {
        loginData.push({ name: "slug_kelas", value: slug }, { name: "main_leads", value: main_leads });
      }
    }
    ...
    $.post($(this).attr("action"), loginData, function (data) {
      ...
      const errorMessageEmail = document.getElementsByClassName("error-message-email");
      const errorMessagepassword = document.getElementsByClassName("error-message-password");

      // check old message error
      if (errorMessageEmail.length > 0) {
        errorMessageEmail[0].parentNode.removeChild(errorMessageEmail[0]);
      }
      if (errorMessagepassword.length > 0) {
        errorMessagepassword[0].parentNode.removeChild(errorMessagepassword[0]);
      }

      var erroJson = JSON.parse(response.responseText);
      for (var err in erroJson) {
        for (var errstr of erroJson[err]) {
          const errorMessage = document.getElementsByClassName("error-message-" + err);
          while (errorMessage.length > 0) {
            errorMessage[0].parentNode.removeChild(errorMessage[0]);
          }

          var errorElement = '<small class="form-text text-muted text-red error-message-' + err + '">' + errstr + "</small>";

...
function scrollToSection(e) {
    ...
    const triggerEl = document.querySelectorAll(
        '#pills-tab button[data-bs-target="#pills-about"]'
    )[0];
...
$(function () {
    ...
    $(".__carousel-preview .flickity-slider img")
        ...
        .on("click", function () {
            $(
                ".__carousel-preview .flickity-slider img:first-child"
            ).removeClass("is-selected is-nav-selected");
        });

    function firstFlickitySlider() {
        //remove active mini carousel
        $(".flickity-slider")
            .children("img.is-selected.is-nav-selected")
            .removeClass("is-selected is-nav-selected");
        $(".__main-preview .flickity-slider")
            .children("img.is-selected")
        ...
        $(".__carousel-preview .flickity-slider img:first-child")
            .addClass("is-selected is-nav-selected")
    ...
    $(document).on("click", ".btn-preview-materi", function () {
        ...
        let urlSrc =
            ...
            video_id +
            "?origin=https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1&autoplay=0";
        var srcVideo =
            '<div class="plyr__video-embed ratio video-place ratio-16x9 sticky-custom" id="player"><iframe src="' +
            urlSrc +
            '" allowfullscreen allowtransparency autoplay="0"></iframe></div>';
        ...
        if ($(window).width() < 576) {
            ...
            $(
                ".__main-preview.course-plyr .flickity-slider #player iframe"
            ).attr("src", urlSrc);
            //Force flickity slider position to left
            firstFlickitySlider();
        } else {
